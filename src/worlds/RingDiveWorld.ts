...
/**
 * RING DIVE — vuelas dentro de los anillos de un gigante gaseoso: una lamina
 * fina de hielo y roca (InstancedMesh reciclado, confinado a y~0) con el
 * planeta rayado de fondo.
 */

const CHUNK_COUNT = 480;
...
export class RingDiveWorld extends World {
  ...
  init(camera: THREE.PerspectiveCamera): void {
    ...
    this.planet.position.set(-350, -40, -900);
  ...
  update(dt: number, _elapsed: number, camera: THREE.PerspectiveCamera): void {
    this.planet.position.set(camera.position.x - 350, -40, camera.position.z - 900);
    for (let i = 0; i < CHUNK_COUNT; i++) {
      const c = this.data[i];
